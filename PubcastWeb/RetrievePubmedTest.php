...
for ($z=0;$z<count($QueryArray);$z++)
{
	echo "In iteration: $z\n";
	 ...
	 $url="https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&term=(".$Pubquery."[Title/Abstract]) AND cancer mutation AND hasabstract&retmax=20";
	 $dom = new DOMDocument();
	 $dom->load($url);
	 $idList = array();
	...
	for ($i=0;$i<$amnt;$i++)
		   ...
		   //First Author Affiliation
			...
			if ($dom->getElementsByTagName("AuthorList")->length>0)
			{
					...
					for ($l=0;$l<$AuthorList->length;$l++)
					{
						...
						$arr = explode("\n", $Length);
						echo count($arr)."\n";
						if(hasChild($AuthorList->item($l))&&count($arr)>=5){
		...
		$query="INSERT INTO Pubcast (PMID,Title,PubDate,Abstract,Authors,Affiliation,Journal,partAbs,Gene) VALUES ('$PMID','$Title','$PubDate','$Abstract','$Authors','$Affiliation','$Journal','$partAbs','$Gene')";
...
function FindGene($Abs,$Genes) {
	...
	for($x = 0; $x < $arrlength; $x++) 
	{
		 ...
		 $Symbol = substr($Symbol, 0, -4);
			for ($y=0;$y<count($NonRedundantGene);$y++)
			{
				$S=ltrim($NonRedundantGene[$y]," ");
				$S=rtrim($S," ");
			//	echo "S ".$S." end\n";
			//	echo "Sym ".$Symbol." end\n";
				if($S==$Symbol)
				...
				//	echo $S
				//	echo "here\n";
				//	echo "non".$NonRedundantGene[$y]."end\n";
;				}
			}
		if ($Symbol!=="" && $Symbol!==" " && strlen($Symbol)>1)
		{			
			if (strpos($Abs, ' '.$Symbol.' ') !== FALSE)
			{	
				$G.=$Symbol." ";
				$NonRedundantGene[]=$Symbol;
			//	echo "non: ".$NonRedundantGene[0]."\n";
			//	echo $G." ".count ($NonRedundantGene)."\n"; 
			}				
		}
